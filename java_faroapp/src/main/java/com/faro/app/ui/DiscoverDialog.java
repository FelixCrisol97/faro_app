...
public final class DiscoverDialog {
    ...
    public static List<DatabaseEntry> show(
            Window owner, CredentialStore credentials, AppPreferences preferences, List<DatabaseEntry> existing) {
        return show(owner, credentials, preferences, existing, null);
    }

    /**
     * {@code existing} son las bases YA registradas — el diálogo las marca como "ya
     * agregada" y no deja volver a agregarlas (ver
     * {@code DiscoverDialogController#alreadyRegistered}). {@code initialHost}
     * precarga el campo de host — usado por "Descubrir bases en esta IP…" del menú
     * contextual de una fila (ver ConnectionTreeCell); {@code null} deja el campo
     * vacío como antes.
     */
    public static List<DatabaseEntry> show(
            Window owner, CredentialStore credentials, AppPreferences preferences,
            List<DatabaseEntry> existing, String initialHost) {
        try {
            ...
            controller.attachExistingDatabases(existing);
            if (initialHost != null) {
