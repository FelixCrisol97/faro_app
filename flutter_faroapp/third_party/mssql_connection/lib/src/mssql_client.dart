...
/// Real duplication fixed 2026-08-03 (AUDITORIA_CODIGO.md): ~12 call sites
/// repeated this exact "if rc != SUCCEED: log, pull the last DB-Library
/// message, throw" sequence, only [logContext] and [defaultMessage] ever
/// differed. A few RPC parameter-binding calls also need to reset RPC state
/// before throwing (otherwise a failed dbrpcparam leaves dbrpcinit unusable
/// for the next attempt) — [onFailure] covers that without forcing every
/// other call site to carry the reset logic too.
void _checkRc(
  ...
  if (rc == SUCCEED) return;
  MssqlLogger.e('$logContext | rc=$rc | error=fail');
  onFailure?.call();
  final em = DBLib.takeLastMessage(dbproc) ?? DBLib.takeLastError(dbproc);
  throw SQLException(em ?? defaultMessage);
}

/// Best-effort: in case a failed RPC call left state dirty, reset it so the
/// next `dbrpcinit` attempt (e.g. a retry, or the next query) isn't blocked
/// by this one's failure.
void _resetRpcState(DBLib db, Pointer<DBPROCESS> dbproc) {
  try {
...
class MssqlClient {
  ...
  /// 6) Free the LOGINREC (dbloginfree) — no longer needed once dbopen
  ///    has returned, success or failure alike.
  ///
    ...
    try {
      ...
      try {
        ...
        if (su != SUCCEED || sp != SUCCEED) {
          ...
          // Real bug fixed 2026-08-04 (AUDITORIA_CODIGO.md): every early
          // return between a successful dblogin() and dbopen() used to
          // leak this LOGINREC — see the note at the dbopen call site
          // below for the general fix; this one exits before ever
          // reaching dbopen(), so it needs its own free.
          _db!.dbloginfree(login);
          return false;
        }
      ...
      // Real bug fixed 2026-08-04 (AUDITORIA_CODIGO.md): `dbloginfree`
      // wasn't even bound (see `freetds_bindings.dart`), so every
      // successful `dblogin()` here leaked its native LOGINREC for the
      // life of the process — one throwaway isolate calls `connect()`
      // once, but a long-running app session opens many of them over
      // time. Safe to free right here regardless of whether `dbopen`
      // itself succeeded or failed: DB-Lib copies whatever it needs from
      // the LOGINREC into the DBPROCESS by the time `dbopen` returns,
      // matching how virtually every DB-Library reference example frees
      // the login immediately after opening.
      _db!.dbloginfree(login);
      ...
      // Real bug fixed 2026-08-04 (AUDITORIA_CODIGO.md): see
      // DBLib.clearMessages's doc comment — a native DBPROCESS address can
      // be reused by a later, unrelated connection once the earlier one is
      // freed; without this, that new connection could inherit a stale
      // PRINT/RAISERROR message left over from whatever last used this
      // address, misattributing it as its own on its first real error.
      DBLib.clearMessages(_dbproc);
  ...
  Future<int> bulkInsert(
    ...
    if (tn.startsWith('#')) {
      ...
      for (final row in rows) {
        ...
        // Real bug fixed 2026-08-04 (AUDITORIA_CODIGO.md): `executeParams`
        // itself wasn't wrapped here — a genuine failure partway through
        // (a dropped connection, not just "this row's data was invalid")
        // used to propagate straight out of `bulkInsert`, discarding
        // `total` entirely and leaving the caller with no idea how many
        // rows had already committed. Deliberately **not** wrapping this
        // loop in one shared transaction instead (which would make it
        // atomic but reverses the intent) — `DbConnector.insertRows`'s own
        // doc comment (the layer that calls into this fallback) explains
        // why every row is executed as its own independent, auto-committed
        // statement on purpose: one bad row must not block the rest, and
        // on Postgres specifically a failed statement inside an explicit
        // transaction poisons every statement after it. This fallback
        // already followed that same one-row-at-a-time contract; it just
        // wasn't resilient to a row's *own* attempt throwing.
        try {
          ...
          // Any failure — a per-row SQL error, a dropped connection, a
          // malformed response — is treated the same way: this row didn't
          // make it, move on to the next one.
        }
      }
      return total;
    }
    ...
    try {
      ...
      // Real bug fixed 2026-08-04 (AUDITORIA_CODIGO.md): a failure anywhere
      // between here and the happy-path `bcp_done` further down
      // (`bcp_bind`, `bcp_sendrow`, `bcp_batch`) used to throw without ever
      // calling `bcp_done` — DB-Lib's BCP state machine is left
      // "in progress" on this connection, since `bcp_init` already
      // succeeded but nothing ever told it the operation was over.
      // Whatever this connection is used for next (another query, another
      // bulk insert) would inherit that dangling state. `bcpFinished`
      // tracks whether the happy path's own `bcp_done` call below already
      // ran, so the `finally` only adds a *second*, best-effort one when
      // an exception cut that path short.
      var bcpFinished = false;
      try {
        // Bind columns with host types; varaddr NULL, varlen -1, no terminator.
        //
        // Real bug fixed 2026-08-03 (AUDITORIA_CODIGO.md): the host type for
        // a column must be decided once, here, before any row is sent —
        // bcp_bind fixes it for the whole batch, there's no per-row rebind.
        // The old code sampled only `rows.first`, so a column whose FIRST
        // row happened to be null always got bound as SYBNVARCHAR (the
        // `_hostTypeFor(null)` fallback) even when every other row held a
        // real int/double/bool/binary value — `_encodeForHost` then
        // stringified those real values (harmless-ish for numbers via
        // SQL Server's own text-to-numeric BCP conversion, but a `Uint8List`
        // gets `.toString()`-ed into literal "[1, 2, 3]" text, silently
        // corrupting binary columns). Fixed by sampling the first *non-null*
        // value across all rows for that column instead — if a column is
        // null in every single row, the host type genuinely doesn't matter
        // (every value takes the NULL branch below, `_encodeForHost` is
        // never reached for it), so falling through to the same
        // SYBNVARCHAR default in that case is still correct.
        final hostTypes = <int>[];
        ...
        if (!bcpFinished) {
          try {
            ...
            // Best-effort cleanup only — the real exception already
            // propagating out is the one the caller needs to see.
          }
        }
      }
    } finally {
  ...
  /// { columns: [..], rows: [ [..], ..], affected: (int), error?: (string) }
  ///
  /// Logging: emits lines in the form `execute | key=value | ...`.
  Future<String> execute(String sql) async => jsonEncode(await _executeRaw(sql));

  /// Same as [execute], but returns the decoded result directly — skips the
  /// `jsonEncode`/`jsonDecode` round trip entirely. Added for the SELECT
  /// path behind large result sets (Faro's `SqlServerConnector`), where
  /// JSON-roundtripping a ~100k-row result set was a real memory/CPU cost;
  /// every other caller (SET-option batches, small administrative queries)
  /// keeps using [execute].
  Future<Map<String, dynamic>> executeRaw(String sql) => _executeRaw(sql);

  Future<Map<String, dynamic>> _executeRaw(String sql) async {
    _ensureConnected();
    final db = _db!;
    final dbproc = _dbproc!;
    ...
    if (plan.needsSet) {
      // Real duplication fixed 2026-08-03 (AUDITORIA_CODIGO.md): this branch
      // and the "regular path" below used to repeat the exact same
      // dbcmd→dbsqlexec→collect sequence three times over (once here for
      // the SET-options batch, once here for the real SQL, once in the
      // no-SET-needed branch), differing only in which SQL text was sent
      // and whether the result got returned or discarded. Both now go
      // through the same [_runBatch] helper.
      // 1) Enable options in their own batch (ensures CREATE VIEW etc. come
      // first, in a batch of their own); its result set is empty, so the
      // return value is discarded.
      _runBatch(db, dbproc, plan.setPrefix, context: ' (SET options)');
      // 2) Execute the original SQL in its own batch.
    }
    return _runBatch(db, dbproc, sql);
  }

  /// Sends [sql] as a single DB-Lib batch (dbcmd → dbsqlexec) and returns its
  /// collected results. Shared by both branches of [_executeRaw] — see the
  /// dedup note there. [context] is appended to the fallback error message
  /// (used only when DB-Lib has no error message of its own to report) so
  /// the SET-options batch keeps a distinguishable error from the real SQL.
  Map<String, dynamic> _runBatch(
      DBLib db, Pointer<DBPROCESS> dbproc, String sql,
      {String context = ''}) {
    final cmd = sql.toNativeUtf8();
    try {
      MssqlLogger.i('execute | op=dbcmd | sqlLen=${sql.length}');
      final rc1 = db.dbcmd(dbproc, cmd);
      _checkRc(dbproc, rc1, 'execute | op=dbcmd', 'dbcmd failed$context');
      MssqlLogger.i('execute | op=dbsqlexec');
      final rc2 = db.dbsqlexec(dbproc);
      _checkRc(
          dbproc, rc2, 'execute | op=dbsqlexec', 'dbsqlexec failed$context');
      return _collectResultsRaw(db, dbproc);
    } finally {
      malloc.free(cmd);
    }
  }
  ...
  Future<String> executeParams(String sql, Map<String, dynamic> params) async {
    ...
    try {
      ...
      _checkRc(dbproc, rcInit, 'executeParams | op=dbrpcinit', 'dbrpcinit failed',
          onFailure: () => _resetRpcState(db, dbproc));
      ...
      final rcP1 = db.dbrpcparam(
        ...
        // datalen: for NVARCHAR, dbrpcparam widens our Latin-1 buffer to
        // UCS-2 itself and expects the *output* byte count (2x the
        // Latin-1 byte length here, since it's 1 byte/char) — see
        // _encodeStringSmart's doc comment. VARCHAR expects raw bytes.
      ...
      _checkRc(dbproc, rcP1, 'executeParams | op=dbrpcparam | name=@stmt',
          'dbrpcparam @stmt failed',
          onFailure: () => _resetRpcState(db, dbproc));
      ...
      final rcP2 = db.dbrpcparam(
        ...
        paramsBuf.type,
        -1, // maxlen: -1 for non-OUTPUT
        // datalen: see the @stmt dbrpcparam call above for why NVARCHAR
        // doubles the (Latin-1, 1 byte/char) buffer length here.
      ...
      _checkRc(dbproc, rcP2, 'executeParams | op=dbrpcparam | name=@params',
          'dbrpcparam @params failed',
          onFailure: () => _resetRpcState(db, dbproc));
      ...
      for (final e in norm.entries) {
        ...
        malloc.free(cname);
        _checkRc(dbproc, rcPi, 'executeParams | op=dbrpcparam | name=$name',
            'dbrpcparam failed',
            onFailure: () => _resetRpcState(db, dbproc));
      }
      ...
      _checkRc(dbproc, rcSend, 'executeParams | op=dbrpcsend', 'dbrpcsend failed');

      MssqlLogger.i('executeParams | op=dbsqlok');
      final rcOk = db.dbsqlok(dbproc);
      _checkRc(dbproc, rcOk, 'executeParams | op=dbsqlok', 'dbsqlok failed');
  ...
  ///
  /// Returns: { columns: [...], rows: [...], affected: (int), error?: (string) }
  /// — see [_collectResults] for the JSON-string-returning wrapper most
  /// callers use, and `executeRaw`/[MssqlClient.executeRaw] for the one that
  /// skips that JSON step entirely.
  Map<String, dynamic> _collectResultsRaw(DBLib db, Pointer<DBPROCESS> dbproc) {
    // Positional, not `Map<String,dynamic>` per row — for a 100k-row result
    // set a hashmap per row was a real memory cost for no benefit, since
    // every row is already in the same order as `columns` below.
      ...
      if (ncols > 0 && capturedFirstSet && columns.isNotEmpty) {
        while (true) {
          ...
          for (var i = 1; i <= ncols; i++) {
            ...
            row[i - 1] = decodeDbValueWithFallback(db, dbproc, t, ptr, len);
          }
          rows.add(row);
          fetched++;
        }
  ...
  /// JSON-string wrapper around [_collectResultsRaw] — used by every caller
  /// except the `executeRaw` SELECT path (see that method's doc comment).
  String _collectResults(DBLib db, Pointer<DBPROCESS> dbproc) =>
      jsonEncode(_collectResultsRaw(db, dbproc));
  ...
  _SetPlan _analyzeSetNeeds(String sql) {
    ...
    final isDdl =
        ...
        up.startsWith('ALTER ') ||
        up.startsWith('DROP ');
    // Real dead code removed 2026-08-03 (AUDITORIA_CODIGO.md): this used to
    // also check a `targetsStrict` flag (CREATE/ALTER VIEW, TABLE, INDEX,
    // FUNCTION, PROCEDURE, TRIGGER) ORed into the condition below. Every one
    // of those prefixes already starts with 'CREATE ' or 'ALTER ', so
    // `targetsStrict` was always a subset of `isDdl` — it could never make
    // the condition true when `isDdl` was false, making the whole variable
    // (and the 12-way startsWith chain computing it) dead weight.
    if (!isDdl) return const _SetPlan(false, '');
    const setPrefix =
        'SET ANSI_NULLS ON; '
...
// Encode a Dart string as either UTF-8 (VARCHAR) if ASCII-only, or a
// FreeTDS-compatible NVARCHAR parameter buffer otherwise.
//
// Real bug found & fixed 2026-08-02 — see AUDITORIA_CODIGO.md /
// CONTEXTO_SESIONES.md for the full live investigation. This function used
// to UTF-16LE-encode non-ASCII strings on the assumption that dbrpcparam
// passes a SYBNVARCHAR value's bytes straight through the wire, with
// `datalen` being either the byte length or the character count of that
// buffer. Neither is true: confirmed empirically against a live SQL
// Server (comparing DATALENGTH()/LEN() and the exact stored UCS-2 code
// units, byte for byte, against several candidate encodings) that
// dbrpcparam does its OWN internal single-byte -> UCS-2 widening of the
// value buffer for SYBNVARCHAR params — `datalen` is the *output*
// (post-widening) byte count, i.e. 2x the number of single-byte source
// characters, and the widening maps each source byte directly to the
// Unicode code point of the same value (0xXX -> U+00XX), which is exactly
// Latin-1 (ISO-8859-1) semantics, not UTF-16LE and not Windows-1252 (whose
// 0x80-0x9F range remaps to different code points, e.g. the Euro sign,
// that a direct byte->codepoint widening can't reach here). Passing a
// UTF-16LE-encoded buffer through this same widening step is what produced
// the original bug: each of *its* bytes (including the interleaved 0x00
// high bytes) got widened again, corrupting the value.
_StringDbBuf _encodeStringSmart(String s) {
  ...
  for (final cu in units) {
    if (cu > 0xFF) {
      // Outside what this RPC-parameter path can transmit correctly (see
      // doc comment above) — throw instead of silently truncating/mangling
      // the value.
      throw SQLException(
        'No se puede enviar el texto a SQL Server: contiene un carácter '
        '(U+${cu.toRadixString(16).padLeft(4, '0')}) fuera del rango '
        'Latin-1 (ISO-8859-1), el único que este driver puede transmitir '
        'como parámetro NVARCHAR.',
      );
    }
  }
