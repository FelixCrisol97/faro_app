...
class ServerNode extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // A server just created by dragging two "Sin grupo" databases together
    // (`ServersNotifier.createServerFromTwoUngroupedDatabases`) starts with
    // a placeholder name — pop the rename dialog open immediately instead
    // of leaving it sitting there as "Nuevo servidor" until someone
    // remembers the pencil icon exists. Scheduled for next frame (not
    // called straight from this listener) since a dialog can't open
    // mid-build.
    ref.listen<String?>(pendingRenameFocusIdProvider, (previous, next) {
      if (next != server.id) return;
      ref.read(pendingRenameFocusIdProvider.notifier).state = null;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (context.mounted) showEditServerDialog(context, ref, server);
      });
    });
    ...
    return Column(
      ...
      children: [
        TreeDropTarget(
          onWillAccept: (payload) => switch (payload) {
            ServerDragPayload() => true,
            // Dropped on this server's own header carries no position
            // info — a no-op there (see this file's drop-matrix doc
            // comment on `_onDrop`); still "accepted" so the row
            // highlights consistently rather than looking broken.
            DatabaseDragPayload() => true,
          },
          onAccept: (payload) => _onDrop(ref, payload),
          child: Material(
            ...
            child: InkWell(
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    TreeDragHandle(
                      payload: ServerDragPayload(server.id),
                      label: server.name,
                    ...
                    Tooltip(
                      ...
                      child: InkWell(
                        borderRadius:
                            const BorderRadius.all(Radius.circular(6)),
                        onTap: () =>
                            showAddDatabaseDialog(context, ref, server),
                        child: Padding(
                          padding: const EdgeInsets.all(4),
                    ...
                    Tooltip(
                      ...
                      child: InkWell(
                        borderRadius:
                            const BorderRadius.all(Radius.circular(6)),
                        onTap: () =>
                            showRemoveServerDialog(context, ref, server),
                        child: Padding(
                          padding: const EdgeInsets.all(4),
              ...
              children: [
                ...
                TreeTrailingDropZone(
                  ...
                  onAccept: (payload) {
                    ...
                    ref.read(serversProvider.notifier).moveDatabaseToServer(
                        p.serverId, p.databaseId, server.id,
                        targetIndex: server.databases.length);
                  },
                ),
              ],
            ),
  ...
  /// Dropped directly on this server's own header row (not on a specific
  /// database row inside it, nor the trailing zone — see `tree_drag.dart`'s
  /// drop-matrix doc comment): a server dropped here reorders relative to
  /// this one; a database dropped here appends into this server unless it
  /// was already here (own header carries no position info, so that case
  /// is a deliberate no-op rather than guessing a target index).
  void _onDrop(WidgetRef ref, TreeDragPayload payload) {
    final notifier = ref.read(serversProvider.notifier);
    switch (payload) {
      case ServerDragPayload(:final serverId):
        if (serverId == server.id) return;
        final index =
            ref.read(serverListProvider).indexWhere((s) => s.id == server.id);
        if (index == -1) return;
        notifier.reorderServer(serverId, index);
      case DatabaseDragPayload(:final serverId, :final databaseId):
        if (serverId == server.id) return;
        notifier.moveDatabaseToServer(serverId, databaseId, server.id);
    }
  }
      ...
      [
        if (!isFirst)
          const PopupMenuItem(
              ...
              child:
                  ContextMenuLabel(LucideIcons.chevrons_up, 'Mover al inicio')),
        if (!isFirst)
          const PopupMenuItem(
