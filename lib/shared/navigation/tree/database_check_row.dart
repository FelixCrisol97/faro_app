...
import '../../../features/consulta/application/query_tabs_providers.dart';
import '../../../features/consulta/application/schema_explorer_provider.dart';
import 'schema_objects_tree.dart';
import 'tree_drag.dart';

/// [serverId]/[server] `null` (2026-08-13) renders a "Sin grupo" database —
/// fully queryable like any other (see [DatabaseEntry.engine]'s doc
/// comment: a database carries its own engine/credentials regardless of
/// grouping), so this row no longer needs a separate, near-duplicate
/// "ungrouped" widget — `ungrouped_section.dart` renders this same class
/// directly. The one action that genuinely can't apply without a server is
/// "Descubrir más bases de datos" (there's no group to add the results
/// into) — hidden from the context menu when [server] is null, everything
/// else (select, test connection, mode, edit, credentials, delete, open in
/// tab/window, browse schema) works identically either way.
...
class _DatabaseCheckRowState extends ConsumerState<DatabaseCheckRow> {
  ...
  /// Dropped directly on this database's own row. Four cases:
  /// - Both this row and the dragged one are "Sin grupo" (`serverId ==
  ///   null` on both sides) — merges the two into a brand-new server
  ///   (same gesture `ungrouped_section.dart`'s trailing drop zone and
  ///   `ServerNode`'s own drop target already use), not a reorder.
  /// - Otherwise, same group on both sides — reorders in place.
  /// - Otherwise — moves the dragged database into/out of "Sin grupo" or
  ///   between two different servers.
  /// The target index is looked up fresh at drop-time (not threaded down
  /// as a prop) from whichever list this row actually belongs to — this
  /// row's own [DatabaseEntry] siblings, correct regardless of whether it
  /// rendered via the flat list, a `HostGroupNode` subset, or
  /// `UngroupedSection`.
  void _onDrop(WidgetRef ref, DatabaseDragPayload payload) {
    if (payload.databaseId == widget.database.id) return;
    final notifier = ref.read(serversProvider.notifier);

    if (widget.serverId == null && payload.serverId == null) {
      final newServerId = notifier.createServerFromTwoUngroupedDatabases(
          payload.databaseId, widget.database.id);
      if (newServerId != null) {
        ref.read(pendingRenameFocusIdProvider.notifier).state = newServerId;
      }
      return;
    }

    final siblings = widget.serverId == null
        ? ref.read(ungroupedDatabasesProvider)
        : widget.server!.databases;
    final targetIndex =
        siblings.indexWhere((db) => db.id == widget.database.id);
    if (targetIndex == -1) return;

    if (payload.serverId == widget.serverId) {
      if (widget.serverId == null) {
        notifier.reorderUngroupedDatabase(payload.databaseId, targetIndex);
      } else {
        notifier.reorderDatabaseWithinServer(
            widget.serverId!, payload.databaseId, targetIndex);
      }
    } else if (widget.serverId == null) {
      notifier.moveDatabaseToUngrouped(payload.serverId!, payload.databaseId,
          targetIndex: targetIndex);
    } else {
      notifier.moveDatabaseToServer(
          payload.serverId, payload.databaseId, widget.serverId!,
          targetIndex: targetIndex);
    }
  }
  ...
  Future<void> _showDatabaseContextMenu(
    ...
    final action = await showPositionedMenu<_DatabaseMenuAction>(
      ...
      [
        ...
        // "Sin grupo" has no natural target group to add discovered
        // databases into — see this class's own doc comment.
        if (widget.server != null)
          const PopupMenuItem(
              value: _DatabaseMenuAction.discoverMore,
              child: ContextMenuLabel(LucideIcons.database_zap,
                  'Descubrir más bases de datos en esta IP')),
      ],
    );
    ...
    switch (action) {
      case _DatabaseMenuAction.openTab:
        ref
            .read(queryTabsProvider.notifier)
            .openTab(serverId: widget.serverId, databaseId: database.id);
  ...
  Widget build(BuildContext context) {
    ...
    final rowColor =
        database.selected ? colors.accent.softText : colors.textMuted;
    ...
    return Column(
      ...
      children: [
        TreeDropTarget(
          ...
          onAccept: (payload) => _onDrop(ref, payload as DatabaseDragPayload),
          child: Material(
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Tooltip(
                      // Real host/IP, on hover — the row itself never shows
                      // this as inline text (2026-07-24: a per-server
                      // subtitle here used to imply a false single-host
                      // identity for servidores spanning several real IPs —
                      // see `HostGroupNode`'s own doc comment). This is
                      // scoped to just this one database though, so it
                      // can't mislead the same way; added 2026-08-19,
                      // user-requested, specifically for the common case
                      // where every database in a servidor *does* share one
                      // host (so `HostGroupNode`'s per-IP grouping never
                      // kicks in and the IP is otherwise invisible without
                      // opening "Editar base de datos").
                      message: database.host.isEmpty
                      ...
                      child: InkWell(
                        ...
                        onSecondaryTapDown: (details) =>
                            tapPosition = details.globalPosition,
                        onSecondaryTap: () =>
                            _showDatabaseContextMenu(tapPosition!, database),
                        child: Text(
                          database.name,
                          overflow: TextOverflow.ellipsis,
                          style: typography.body.copyWith(
                            fontSize: 13,
                            color: database.selected
                                ? colors.accent.softText
                                : colors.text,
                          ),
                        ),
                      ),
                  ...
                  Tooltip(
                    message:
                        'Credenciales solo para esta base de datos (opcional)',
                    child: InkWell(
                      borderRadius: const BorderRadius.all(Radius.circular(6)),
        ...
        if (_expanded)
          Padding(
            ...
            child: _loadRequested
                ? SchemaObjectsTree(dbKey: _key, engine: database.engine)
  ...
  Future<void> _onToggleMode(BuildContext context, WidgetRef ref) async {
    ...
    if (database.mode == ServerMode.development) {
      notifier.setDatabaseMode(
          widget.serverId, database.id, ServerMode.readOnly);
      return;
    }
